...
namespace Interaction
{
    internal partial class Publisher
    {
        ...
        public Publisher(IConfiguration configuration)
        {
            ...
            ClientID = configuration.GetValue<string>("Forge:ClientId");
            ClientSecret = configuration.GetValue<string>("Forge:ClientSecret");
        }
        ...
        {
            dynamic oauth = await GetInternalTokenAsync();

            // create work item
            var wi = new WorkItem
            {
                ActivityId = await GetFullActivityId(),
                Arguments = GetWorkItemArgs(BucketName, oauth.access_token)
            };
        ...
        public static async Task<dynamic> GetInternalTokenAsync()
        {
            if (_token == null || _token.ExpiresAt < DateTime.UtcNow)
            {
                _token = await Get2LeggedTokenAsync(new Scope[] { Scope.BucketCreate, Scope.BucketRead, Scope.BucketDelete, Scope.DataRead, Scope.DataWrite, Scope.DataCreate, Scope.CodeAll });
                _token.ExpiresAt = DateTime.UtcNow.AddSeconds(_token.expires_in);
            }
